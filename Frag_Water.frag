...
in vec3 vs_out_pos;
in vec3 vs_out_norm; // nekünk ez most nem kell...
in vec2 vs_out_tex;
...
// spot fény tulajdonságok

uniform bool  isSpotLight = false;
uniform float spotCosCutoff = -1.0;
uniform float spotExponent  =  0.0;
uniform vec3  spotDirection = vec3(0.0,-1.0,0.0);
...
void main()
{
	...
	vec3 normal = normalize( vs_out_norm + (texture(texNormalMap, vs_out_tex).xzy * vec3( 2.0, 2.0, -2.0) - vec3(1.0, 1.0, -1.0)) );
